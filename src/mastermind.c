...
//-------------------------GPSET(DigitalWrite) FOR LCD(ASSEMBLER)---------------
...
void compare(struct lcdDataStruct *lcd, int guessNo){
    ...
    /* store the secret in a temporary array, this allows to calculate the
...
int main (int argc, char *argv[]) {
    ...
    lcdPosition (lcd, 0, 0) ; lcdPuts (lcd,"            ");                         // writing to the LCD so the clear isn't clearing nothing - this was giving issues before
    lcdClear       (lcd) ;
